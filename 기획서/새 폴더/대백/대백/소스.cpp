...
void ip() { //ip주소 가져오기
	...
	mysql_query(conn, "select ip from user");
	sql_result = mysql_store_result(conn);
	while ((sql_row = mysql_fetch_row(sql_result)) != NULL) {
		if (strcmp(sql_row[0], ip) == 0) return;
	}
	sprintf(query, "insert into user (name,ip) values ('%s','%s')", name[rand() % 12], ip);
